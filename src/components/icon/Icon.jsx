...
const RefWrapper = forwardRef(({ children }, ref) => {
    if (ref) {
        return (<span ref={ref} data-only-ref='true'>
				{children}
			</span>);
    }
    return children;
...
const Icon = forwardRef(({ icon, className, color, size, forceFamily, ...props }, ref) => {
    ...
    const _className = classNames('svg-icon', { [`svg-icon-${size}`]: size, [`text-${color}`]: color }, className);
    ...
    if (isForceCustom ||
        ...
        return (<RefWrapper ref={ref}>
					<SvgIconWrapper data-name={`SvgIcon--${_icon}`} className={classNames('svg-icon--custom', _className)} {...props}/>
				</RefWrapper>);
    }
    if (isForceMaterial ||
        (!isForceCustom && !isForceBootstrap && typeof MaterialWrapper === 'function')) {
        return (<RefWrapper ref={ref}>
					<MaterialWrapper data-name={`Material--${icon}`} className={classNames('svg-icon--material', _className)} {...props}/>
				</RefWrapper>);
    }
    if (isForceBootstrap ||
        (!isForceCustom && !isForceMaterial && typeof BootstrapWrapper === 'function')) {
        return (<RefWrapper ref={ref}>
					<BootstrapWrapper data-name={`Bootstrap--${_icon}`} className={classNames('svg-icon--bootstrap', _className)} {...props}/>
				</RefWrapper>);
    }
    return null;
});
