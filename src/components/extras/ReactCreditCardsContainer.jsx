...
const ReactCreditCardsContainer = ({ className, is3dShadow, issuer, scale, children, ...props }) => {
    const [ref, { width }] = useMeasure();
    // @ts-ignore
    const classes = useStyles({ width, scale });
    return (<div ref={ref} className={classNames({
            [`rccs-shadow-3d-${(issuer === 'visa' && 'info') ||
                (issuer === 'mastercard' && 'warning') ||
                (issuer === 'amex' && 'success') ||
                (issuer === 'hipercard' && 'danger') ||
                'dark'}`]: is3dShadow,
        }, { [classes.scale]: !!scale, [classes.fluid]: width < 290 }, className)} 
