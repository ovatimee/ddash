...
export const AccordionItem = forwardRef(({ id, icon, title, children, tag, headerTag, overWriteColor, ...props }, ref) => {
    // eslint-disable-next-line react/prop-types
    const ACTIVE = props.activeItem === id;
    return (<TagWrapper tag={tag} ref={ref} className={classNames('accordion-item')}>
				<TagWrapper tag={headerTag} className={classNames('accordion-header')} id={id}>
					<button className={classNames('accordion-button', {
            collapsed: !ACTIVE,
            [`accordion-button-${overWriteColor}`]: overWriteColor,
        })} type='button' data-bs-toggle='collapse' data-bs-target={`#${id}Collapse`} aria-expanded={ACTIVE} aria-controls={`${id}Collapse`} onClick={() => 
        // eslint-disable-next-line react/prop-types
        ACTIVE ? props.setActiveItem(null) : props.setActiveItem(id)}>
				...
				<Collapse isOpen={ACTIVE} id={`${id}Collapse`} className={classNames('accordion-collapse')} aria-labelledby={id} data-bs-parent={`#${props.parentId}`}>
...
const Accordion = forwardRef(({ tag, id, activeItemId, children, shadow, color, isFlush, className }, ref) => {
    const [activeItem, setActiveItem] = useState(activeItemId === false
        ? null
        : activeItemId || (Array.isArray(children) && children?.flat()[0].props.id));
    return (<TagWrapper tag={tag} ref={ref} className={classNames('accordion', {
        ...
        // @ts-ignore
        ['AccordionItem'].includes(child?.type?.displayName) ? (cloneElement(child, {
            activeItem,
            setActiveItem,
            parentId: id,
