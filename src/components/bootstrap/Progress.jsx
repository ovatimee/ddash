...
const Progress = forwardRef(({ value, min, max, height, isStriped, isAnimated, isAutoColor, color, children, className, 
...
isOnlyBar, ...props }, ref) => {
    ...
    const ONLY_BAR = (<div style={{
        ...
        })} role='progressbar' aria-label={`${value}%`} aria-valuenow={value} aria-valuemin={min} aria-valuemax={max}/>);
    if (isOnlyBar) {
        return ONLY_BAR;
    }
    return (<div ref={ref} className={classNames('progress', { [classes.dynamicHeight]: !!height }, className)} style={{
            // @ts-ignore
            // eslint-disable-next-line react/prop-types
            ...props.style,
        }} 
