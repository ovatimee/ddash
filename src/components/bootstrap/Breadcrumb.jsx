...
export const BreadcrumbItem = ({ children, ariaLabel, className, tag, to, isActive, divider, }) => {
    return (<TagWrapper tag={tag} className={classNames('breadcrumb-item', { active: isActive }, className)} aria-current={isActive ? 'page' : null} aria-label={ariaLabel || children}>
			{divider &&
            typeof divider !== 'string' &&
            cloneElement(divider, {
                className: classNames('breadcrumb-icon', divider.props.className),
            })}
			{isActive ? (children) : (<NavLink to={to} aria-label={ariaLabel}>
					{children}
				</NavLink>)}
		</TagWrapper>);
};
...
const Breadcrumb = ({ children, list, tag, listTag, itemTag, ariaLabel, autoActive, isToHome, divider, }) => {
    const DIVIDER = divider !== 'string' && divider;
    return (<TagWrapper tag={tag} aria-label={ariaLabel} style={divider
            ? {
                '--bs-breadcrumb-divider': typeof divider === 'string' ? `'${divider}'` : 'none',
            }
            : null}>
			<TagWrapper tag={listTag} className='breadcrumb'>
				{isToHome && (<BreadcrumbItem to='/' ariaLabel='Home'>
						{isToHome}
					</BreadcrumbItem>)}
				{list
            ? list.map((item, index) => (<BreadcrumbItem key={item.title} tag={item.tag || itemTag} to={item.to} isActive={autoActive && list.length === index + 1} divider={DIVIDER}>
								{item.title}
							</BreadcrumbItem>))
