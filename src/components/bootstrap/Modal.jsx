...
export const ModalTitle = forwardRef(({ tag, id, children, className, ...props }, ref) => {
    return (<TagWrapper tag={tag} ref={ref} id={id} className={classNames('modal-title', className)} 
...
export const ModalHeader = forwardRef(({ children, className, setIsOpen, ...props }, ref) => {
    ...
    <div ref={ref} className={classNames('modal-header', className)} {...props}>
				{children}
				{setIsOpen && (<button type='button' className='btn-close' data-bs-dismiss='modal' aria-label='Close' onClick={() => setIsOpen(false)}/>)}
			</div>);
});
...
const Modal = ({ children, isOpen, setIsOpen, id, titleId, isStaticBackdrop, isScrollable, isCentered, size, fullScreen, isAnimation, ...props }) => {
    ...
    return (<Portal>
			<AnimatePresence exitBeforeEnter>
				{isOpen && (<>
						<motion.div ref={refModal} key='modal' className={classNames('modal', { fade: isAnimation }, 'show')} role='dialog' style={{ display: 'block' }} id={id} tabIndex={-1} aria-labelledby={titleId} aria-hidden='true' data-bs-backdrop={isStaticBackdrop ? 'static' : null} data-bs-keyboard={isStaticBackdrop ? 'false' : null} 
        // eslint-disable-next-line react/jsx-props-no-spreading
        {...ANIMATION_PROPS} 
        // eslint-disable-next-line react/jsx-props-no-spreading
        {...props}>
							<div ref={ref} className={classNames('modal-dialog', {
                'modal-dialog-scrollable': isScrollable,
                'modal-dialog-centered': isCentered,
                [`modal-${size}`]: size,
                [`modal-fullscreen${typeof fullScreen === 'string' ? `-${fullScreen}-down` : ''}`]: fullScreen,
            })}>
