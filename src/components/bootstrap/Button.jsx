...
export const ButtonGroup = forwardRef(({ children, className, isToolbar, isVertical, size, ariaLabel, ...props }, ref) => {
    ...
    return (<div ref={ref} className={classNames({
            ...
            [`btn-group-${size}`]: size,
        }, className)} role={PREFIX} aria-label={ariaLabel} 
...
const Button = forwardRef(({ children, tag, type, to, href, isActive, color, isOutline, isLight, isLink, className, icon, rounded, size, isDisable, shadow, hoverShadow, target, isVisuallyHidden, ...props }, ref) => {
    const BTN_CLASS = classNames('btn', {
        [`btn-${isOutline || isLink ? `outline-${color}` : color}`]: (color && !isLight) || (color && isLink),
    ...
    if (tag === 'a') {
        if (typeof to === 'string' && ANCHOR_LINK_PATTERN.test(to)) {
            return (<HashLink ref={ref} className={BTN_CLASS} to={to} 
            // eslint-disable-next-line react/jsx-props-no-spreading
            {...disableProps} 
            // eslint-disable-next-line react/jsx-props-no-spreading
            {...props}>
        ...
        if (to) {
            return (<Link ref={ref} className={BTN_CLASS} to={to} rel='noopener' target={target} 
            // eslint-disable-next-line react/jsx-props-no-spreading
            {...disableProps} 
            // eslint-disable-next-line react/jsx-props-no-spreading
            {...props}>
        ...
        return (<a ref={ref} className={BTN_CLASS} href={href} role='button' rel='noopener' target={target} 
        // eslint-disable-next-line react/jsx-props-no-spreading
        {...disableProps} 
        // eslint-disable-next-line react/jsx-props-no-spreading
        {...props}>
    ...
    return (<TagWrapper ref={ref} tag={tag} type={type} className={BTN_CLASS} 
    // eslint-disable-next-line react/jsx-props-no-spreading
    {...disableProps} 
    // eslint-disable-next-line react/jsx-props-no-spreading
    {...props}>
