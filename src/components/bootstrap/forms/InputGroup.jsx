...
export const InputGroupText = forwardRef(({ tag, id, className, children, ...props }, ref) => {
    return (<TagWrapper tag={tag} ref={ref} id={id} className={classNames('input-group-text', className)} 
...
const InputGroup = forwardRef(({ id, className, children, isWrap, size, ...props }, ref) => {
    ...
    validClass(children);
    return (<div ref={ref} id={id} className={classNames('input-group', {
            'flex-nowrap': !isWrap,
            [`input-group-${size}`]: size,
            'has-validation': (!IS_VALID && IS_TOUCHED && (INVALID_FEEDBACK || VALID_FEEDBACK)) ||
    ...
    {...props}>
        ...
        item?.props?.isValidMessage
            ...
            : // @ts-ignore
				...
				<Validation isTouched={IS_TOUCHED} validFeedback={VALID_FEEDBACK} invalidFeedback={INVALID_FEEDBACK} isTooltip={IS_TOOLTIP_FEEDBACK}/>
			</div>);
});
